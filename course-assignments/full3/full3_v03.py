...
def positive_t(sptab_ratio):
    ...
    while stop == False:
        ...
        if line == EOF:
            ...
            print(line)
            print("Replaced:")
            for ch in line: # for each character in a line
                if ord(ch) >= 33: # if the character is a letter, stop
                    break
                elif ord(ch) == 9: # else-if the character is a space
                    while (len(result) % sptab_ratio != 0):
                        # while the length of the new string, modulo ratio
                        # is not equal to zero
                        result += chr(32) # add a space to the result
                else:
                    result += ch
            new_line = result + line.lstrip()
            print(new_line)
            print("----------------")
            """
            maybe try something like line[0:len(lstrip)]
            length of indent is len(line) - len(line.lstrip())
            """
"""
...
def positive_v():
    ...
    while stop == False:
        ...
        if line == EOF:
            ...
            print(line)
            print("Replaced:")
            result = line.replace(chr(32), chr(183))
            result = line.replace(chr(9), chr(187)) + chr(9)
            print(result)
...
def main():
    ...
    for arg in sys.argv:
        if firstArg:
            ...
            positive_t(sptab_ratio)
        elif ("-T" in arg):
            sptab_ratio = upper_t(arg)
        elif (arg == "-v"):
            negative_v()
        elif (arg == "+v"):
            positive_v()
