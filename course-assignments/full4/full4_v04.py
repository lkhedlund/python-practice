...
def agent_movement(board, agentLocation, agent_num):
    ...
    can_move = False
    # Agent's Location
    debugPrint("Agent's (r/c): (%r/%r)" % (currentRow, currentColumn))
    # Movement input
...
def freezing_time(articTime):
    ...
    print("Homer has been in the artic for %i days." % articTime)
    print("You have %i days to leave before Homer freezes" % time_left)
...
def error(code, function):
    ...
    if code == "npc":
        print("Agent move value caused a critical failure")
    elif code == "pc":
        print("Homer's move value caused a critical failure")
    exit(0)

def surrounding_space(board, currentRow, currentColumn):
...
def start():
    ...
    board, agentLocation = initialize_world()
